...
parser.add_argument(
    "--MSprint", action="store_true", help="if given prints cluster stats."
)
###
parser.add_argument(
    "--PCvar",
    type=float,
    default=0,
    help="if above 0 retain dimensions as a function of PC variance",
)
###
parser.add_argument(
    "--VARprint",
    action="store_true",
    help="if given prints PC explained variance per window. If PCA is not chosen just prints out 0's",
)
###
parser.add_argument(
    "--id",
    type=str,
    default="1",
    help="Give your analysis an ID. default is set to integer 2",
)
###
...
parser.add_argument(
    "--clustmethod",
    default="MeanShift",
    choices=["MeanShift", "DBscan", "HDBscan"],
    help="Clustering method to extract reference specific clusters. MS, dbscan and hdbsan available. MS preferred",
)
###
parser.add_argument("--het", type=float, default=5e-2, help="Heterozygosity filter")
###
parser.add_argument(
    "--dr", default="NMF", help="Dimensionality reduction. options: PCA, NMF"
)
###
parser.add_argument(
    "--mono", action="store_false", help="if given includes monomorphic SNPs"
)
###
parser.add_argument(
    "--ncomp",
    type=int,
    default=5,
    help="Number of components kept in case of PCA reduction",
)
###
parser.add_argument(
    "--outmethod",
    default="None",
    help="Outlier filter of population refs method. options: DBSCAN, NMF, Perc, Z, Isolation_forest.",
)
###
parser.add_argument(
    "--overlap", type=int, default=100, help="Overlap between windows, in snps"
)
###
...
def Main_engine(Fam, MissG, Geneo, Parents, GenoSUF, CHR, start, end, args):
    ...
    for line in Geno:
        ...
        if (
            ...
            continue
        if len(
            list(set([x for x in Whose if line[x] not in [args.het_cd, args.miss_cd]]))
        ) == int(args.mono):
            Index += 1
            continue
        ...
        if Index >= start and Index <= end:
            ...
            if Win == Window:
                ...
                Daddy = np.array(
                    [Seq[x] for x in it.chain(*[Geneo[x] for x in Parents])]
                )
                ...
                if DIMr == "PCA":
                    pca = PCA(
                        n_components=n_comp, whiten=False, svd_solver="randomized"
                    ).fit(data)
                ...
                if DIMr == "NMF":
                    ...
                    data = NMF(
                        n_components=n_comp, init="random", random_state=0
                    ).fit_transform(data)

                Accurate = []
                params = {
                    "bandwidth": np.linspace(
                        np.min(data), np.max(data), Bandwidth_split
                    )
                }
                grid = GridSearchCV(
                    KernelDensity(algorithm="ball_tree", breadth_first=False),
                    params,
                    verbose=0,
                )
                ...
                if Method == "MeanShift":
                    ...
                    bandwidth = estimate_bandwidth(
                        data[Focus_labels, :], quantile=0.2, n_samples=len(Focus_labels)
                    )
                    if bandwidth <= 1e-3:
                        bandwidth = 0.1

                    ms = MeanShift(
                        bandwidth=bandwidth, cluster_all=False, min_bin_freq=25
                    )
                ...
                for D in range(len(Parents)):
                    Where = [
                        sum([len(Geneo[x]) for x in Parents[0:D]]) + Aro.shape[0],
                        sum([len(Geneo[x]) for x in Parents[0 : (D + 1)]])
                        + Aro.shape[0],
                    ]
                    ...
                    if Outlier_method == "Perc":
                        ...
                        y_pred = [
                            int(x < stats.scoreatpercentile(P_dist, quantile_filter_1))
                            for x in P_dist
                        ]
                    ...
                    if Outlier_method == "MS":
                        ...
                        if len(Set_ref) == 1:
                            ...
                            bandwidth = estimate_bandwidth(
                                Quanted_set, quantile=0.5, n_samples=len(Quanted_set)
                            )
                            if bandwidth == 0:
                                bandwidth = 0.1
                            ms = MeanShift(
                    ...
                    if Outlier_method == "Z":
                        ...
                        y_pred = scipy.stats.norm(mean(P_dist), np.std(P_dist)).cdf(
                            P_dist
                        )
                    ...
                    if Outlier_method == "Isolation_forest":
                        ...
                        Quanted_set = data[Where[0] : Where[1], :]
                        clf = IsolationForest(
                            max_samples=len(Quanted_set), random_state=rng
                        )
                        clf.fit(Quanted_set)
                    ...
                    if sum(np.isnan(P_dist)) == len(P_dist):
                        if len(Likes[D]) == 0:
                            Likes[D].append(
                                [
                                    int(x in range(Where[0], Where[1]))
                    ...
                    if normalize == "CDF":
                        if np.std(P_dist) == 0:
                            Fist = np.array(
                                [int(Fist[x] in P_dist) for x in range(len(Fist))]
                            )
                        else:
                            Fist = scipy.stats.norm(
                                np.mean(P_dist), np.std(P_dist)
                            ).pdf(Fist)
                    else:
                        Fist = kde(data.T)
                        Fist = Fist / max(Fist)
...
for setting in parameters:
    listJobs.append(
        [Fam, MissG, Geneo, Parents, GenoSUF, setting[0], setting[1], setting[2], args]
    )
...
if args.VARprint == True:
    Output = open(
        Home + "Blocks_ExVAR_st" + str(start) + "_CHR" + str(CHR).zfill(2) + ".txt", "w"
    )

    Output.write(
        "CHR\tIN\t" + "\t".join(["PC{0}".format(x + 1) for x in range(args.ncomp)])
    )
