...
def local_sampling_correct(data):
    ...
    grid = GridSearchCV(
        KernelDensity(algorithm="ball_tree", breadth_first=False), params, verbose=0
    )

    ## perform MeanShift clustering.
    ms = MeanShift(
        bandwidth=bandwidth, bin_seeding=True, cluster_all=False, min_bin_freq=20
    )
    ms.fit(features)
    labels1 = ms.labels_
    label_select = {
        y: [x for x in range(len(labels1)) if labels1[x] == y]
        for y in sorted(list(set(labels1)))
    }
...
def readSIM_conf(Fconf):
    ...
    for line in Fconf:
        ...
        if line[0] == "mod.n.samplePerPop":
            ...
            Pops = {
                x + 1: [y for y in range(sum(Pops[:x]), sum(Pops[: x + 1]))]
