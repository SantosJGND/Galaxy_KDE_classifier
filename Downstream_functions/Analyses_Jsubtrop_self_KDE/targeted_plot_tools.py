...
def plot_countries(selected_column,threshold= 0.05,df= {},vectors= {},orderCore= {},allow_geo= [],scope= 'asia',change_name= {}):
    ...
    lik_country= {
        z: [x for x in vectors.iloc[coords[z],(selected_column - 1)] if x >= threshold] for z in allow_geo
    }
    lik_country= {z:lik_country[z] for z in lik_country.keys() if lik_country[z]}
        
    lik_country= {
        z: np.mean(lik_country[z]) for z in lik_country.keys()
    }
