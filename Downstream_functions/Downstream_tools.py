...
def read_coords_dict(coords_file,codes):
    ...
    for line in Input:
        ...
        if d== 0:
            cols= line
            Guides= {x:line.index(x) for x in line}
            d += 1
            print(line)
        else:
            if int(line[Guides['label']]) in codes:
                CHR= int(line[Guides['chrom']])
                window= int(line[Guides['start']])
                cl= int(line[Guides['cluster']])
                label= int(line[Guides['label']])
                
                stock[CHR][window][cl]= label
    ...
    for line in Input:
        ...
        if d== 0:
            ...
            for c in range(len(line)):
                ...
                if cols[c] != 'members':
                    value= int(value)
                stock[cols[c]].append(value)
    
    Input.close()
    
    label_indexes= {z:[x for x in range(len(stock['label'])) if stock['label'][x] == z] for z in list(set(stock['label']))}
    order_by_label= {lab:[[stock['chrom'][x],stock['start'][x],stock['end'][x],stock['cluster'][x],lab] for x in label_indexes[lab]] for lab in label_indexes.keys()}
